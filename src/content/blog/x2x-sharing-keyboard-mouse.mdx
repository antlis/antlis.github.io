...
- **[KDE Connect](https://kdeconnect.kde.org/)** — can use a phone as a touchpad/keyboard and does plenty of other cross-device things, but it is built for the "phone ↔ computer" pairing, not "two desktop X displays side by side."
...
The Linux community is working on standardized protocols (like XDG-foreign) to allow controlled input and clipboard sharing between Wayland compositors, but nothing stable or widely available yet. Another option worth watching is [rkvm](https://github.com/htrefil/rkvm), a Wayland-native input sharing tool written in Rust.
