...
I wrote a [previous post](/blog/mpv-telegram-controller-hermes) about driving mpv from Telegram via the Hermes agent on a `$12/month` nanogpt subscription. The TL;DR there was: it's great, runs through nanogpt's `hermes-medium`, costs me roughly the price of a sandwich per month.
...
If you only use Hermes as a Telegram remote (`/mpv_play lain`, `/mpv_pause` — the [zero-token quick commands](/blog/mpv-telegram-controller-hermes)), you'll never notice the weekly cap. The quick-command path skips the model entirely, so it contributes zero to the 60M counter.
...
Anything that doesn't need natural language should be a quick command. They cost zero tokens. The mpv setup I wrote about uses [13 quick commands](/blog/mpv-telegram-controller-hermes#wiring-it-to-telegram-quick-commands) for things that would otherwise route through the model. If you're calling Hermes for the same operation more than 3 times, write a shell script and wire it as a `quick_commands:` entry.
