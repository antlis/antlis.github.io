...
function term({ pal, path, lines, focus }) {
  ...
  const winW = 760, winX = (W - winW) / 2, winY = 96, winH = H - 200, r = 20
  const barH = 60
  const bodyX = winX + 48
...
const ARTICLES = [
  {
    slug: 'brave-fzf-history', pal: 'shellTeal', path: 'brave-history — fzf',
    lines: [
  ...
  {
    slug: 'fzf-tmuxinator-projects', pal: 'shellGreen', path: 'tmuxinator — fzf',
    lines: [
  ...
  {
    slug: 'linkedin-defi-scammers', pal: 'security', path: 'inbox — recruiter',
    lines: [
  ...
  {
    slug: 'docker-btrfs-disk-recovery', pal: 'dockerBlue', path: '~ — thinkpad',
    lines: [
  ...
  {
    slug: 'hermes-status-bar-token-tweaks', pal: 'hermes', path: 'hermes — status',
    lines: [
  ...
  {
    slug: 'tg-mpv-bot', pal: 'mpv', path: 'tg-mpv-bot — telegram',
    lines: [
  ...
  {
    slug: 'tg-media-bot', pal: 'media', path: 'tg-media-bot — telegram',
    lines: [
  ...
  {
    slug: 'telescope-gist', pal: 'nvim', path: 'nvim — telescope gist',
    lines: [
...
for (const a of ARTICLES) {
  const svg = term(a)
  await sharp(Buffer.from(svg), { density: 144 }).resize(W, H).png().toFile(join(OUT, `${a.slug}.png`))
}
